...
class Paper:
    ...
    def __init__(self, content, nber_id):
        # content is the HTML text for the corresponding NBER paper.
        self.content = content
        self.nber_id = nber_id
    
    def to_date(self, x):
        """Returns to date format for several functions, such as citation_publication_date, issue_date, and revision_date.
        """
        
        return date(x.year, x.month, x.day)
    
    def citation_title(self):
        """Returns the paper title if any. Returns nothing if empty.
        """
    ...
    def citation_publication_date(self):
        """Returns the publication date if any. Returns nothing if empty.
        """
    ...
    def issue_date(self):
        """Returns the issuance date if any. Returns nothing if empty.
        """
    ...
    def revision_date(self):
        """Returns the revision date if any. Returns nothing if empty.
        """
    ...
    def related_title(self):
        """Returns the text that comes from the related method. Returns nothing if empty.
        """
    ...
    def abstract(self):
        """Returns the abstract of the corresponding paper. Returns nothing if empty."""
    ...
    def acknowledgement(self):
        """Returns the acknowledgement if any. Returns nothing if empty.
        """
...
def main(start, end):
    while start < end:
        ...
        if not file_check:
            try:
                print(f"{raw.timestamp()} {raw.url()} [DOWNLOAD \U0001F4BE]: {raw.url()}")
                content = raw.content()
                paper = Paper(content, raw.nber_id)
                paper.save()
                print(f"{raw.timestamp()} {raw.url()} [SUCCEED \U00002705]: {raw.url()}")
            except Exception as err:
                print(traceback.print_exc())
                print(f"{err}: {start}")
                pass
        else:
            print(f"{raw.timestamp()} {raw.url()} [IGNORE \U0001F4C1]: {raw.url()}")
